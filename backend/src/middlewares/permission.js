...
const checkPermission = (resourceOrPermission, action, checkOwnership = false) => {
  return async (req, res, next) => {
    try {
      ...
      if (resourceOrPermission && resourceOrPermission.includes(':')) {
        // 新格式: 'resource:action'
      ...
      logger.debug(`Permission check: resourceOrPermission=${resourceOrPermission}, resource=${resource}, action=${actualAction}`);
