...
class GenericController {
  ...
  /**
   * 下载导入模板
   * GET /api/:modulePath/download-template
   */
  async downloadTemplate(req, res, next) {
    try {
      ...
      const template = await genericService.downloadTemplate(moduleConfig);
  ...
  async importFromExcel(req, res, next) {
    try {
      ...
      const result = await genericService.importFromExcel(moduleConfig, rows);
