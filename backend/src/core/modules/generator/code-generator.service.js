...
class CodeGeneratorService {
  ...
  async generateCode(moduleId, options = {}) {
    try {
      ...
      // 验证表是否有必需的时间戳字段
      await this._validateTimestampColumns(moduleConfig.table_name, tableSchema);
   ...
   * @param {Array} tableSchema - 表结构
   */
  async _validateTimestampColumns(tableName, tableSchema) {
    ...
    if (missingFields.length > 0) {
      const errorMessage = `表 "${tableName}" 缺少必需的时间戳字段: ${missingFields.join(', ')}。请先在表中添加这些字段。`;
      logger.error(errorMessage);
      throw ApiError.badRequest(errorMessage);
    }

    logger.info(`✓ Table ${tableName} has required timestamp columns`);
  }
