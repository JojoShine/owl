...
const moduleConfigService = require('../core/modules/generator/module-config.service');
const genericController = require('../core/modules/generator/generic.controller');
...
async function loadModuleConfig(req, res, next) {
  try {
    ...
    logger.debug(`Module config loaded for: ${modulePath}, permissionPrefix: ${req.permissionPrefix}`);
...
/**
 * 下载导入模板（需要放在 /:id 之前，避免 download-template 被当作 id）
 */
router.get(
  '/:modulePath/download-template',
  (req, res, next) => checkPermission(`${req.permissionPrefix}:read`)(req, res, next),
  genericController.downloadTemplate.bind(genericController)
);
